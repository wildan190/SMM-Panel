...
class Api_order extends MY_Controller
{
	...
	public function index()
	{
		...
		// SETTINGS //
		$data_query = [
			'select' => 'api_order.*, api.name',
			'order_by' => 'api_order.id DESC',
			'join' => [
				[
					'table' => 'api',
					'on' => 'api.id = api_order.api_id',
					'param' => 'inner'
				]
			],
			'limit' => '30',
		...
		$config['total_rows'] = $this->api_order_model->get_count($data_query);
		$config['per_page'] = $data_query['limit'];
		$this->pagination->initialize($config);
		// END PAGINATION //
		$this->render_admin('admin/' . $this->uri->segment(2) . '/index', ['table' => $this->api_order_model->get_rows($data_query), 'total_data' => $config['total_rows'], 'field' => $field, 'operator' => $operator]);
	}
	public function form($i = '')
	{
		...
		if ($target == false) { // add
			if ($this->input->post()) {
				...
				$this->form_validation->set_rules('order_id_key', 'Respond ID Pesanan', 'required');
				$data_input = [
					...
					'order_id_key' => $this->input->post('order_id_key'),
				];
				if ($this->form_validation->run() == false) {
					...
					if ($this->api_model->get_row(['id' => $data_input['api_id']]) == false) {
						$this->session->set_flashdata('result', array('alert' => 'danger', 'title' => 'Gagal!', 'msg' => 'Nama tidak ada didatabase.'));
					} elseif ($this->api_order_model->get_row(['api_id' => $data_input['api_id']])) {
						$this->session->set_flashdata('result', array('alert' => 'danger', 'title' => 'Gagal!', 'msg' => 'Nama sudah ada didatabase.'));
					} else {
						$insert_data = $this->api_order_model->insert($data_input);
						if ($insert_data) {
							...
							$this->session->set_flashdata('result', array('alert' => 'success', 'title' => 'Yeay!', 'msg' => 'Paramater Order Provider <b>#' . $data_input['api_id'] . '</b> berhasil ditambahkan.'));
			...
			if ($this->input->post()) {
				...
				if ($this->form_validation->run() == false) {
					...
					if ($this->api_model->get_row(['id' => $data_input['api_id']]) == false) {
						...
						$this->api_order_model->update($data_input, ['api_id' => $target->api_id]);
						...
						$this->session->set_flashdata('result', array('alert' => 'success', 'title' => 'Yeay!', 'msg' => 'Parameter Order Provider <b>#' . $target->api_id . '</b> berhasil diperbaharui.'));
						redirect(base_url('admin/api'));
						exit();
					}
				}
			...
			$this->load->view('admin/' . $this->uri->segment(2) . '/edit', ['target' => $target, 'api' => $this->api_model->get_rows(), 'api_request_param' => $this->api_request_param_model->get_rows(['where' => [['api_id' => $i, 'api_type' => 'order']]])]);
			// $this->render_admin('admin/' . $this->uri->segment(2) . '/edit', ['target' => $target, 'api' => $this->api_model->get_rows(), 'api_request_param' => $this->api_request_param_model->get_rows(['where' => [['api_id' => $i, 'api_type' => 'order']]])]);
		}
	}
