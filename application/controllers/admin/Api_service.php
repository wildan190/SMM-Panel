...
class Api_service extends MY_Controller
{
	...
	public function index()
	{
		...
		// SETTINGS //
		$data_query = [
			'select' => 'api_service.*, api.name',
			'order_by' => 'api_service.id DESC',
			'join' => [
				[
					'table' => 'api',
					'on' => 'api.id = api_service.api_id',
					'param' => 'inner'
				]
			],
			'limit' => '30',
		...
		$config['total_rows'] = $this->api_service_model->get_count($data_query);
		$config['per_page'] = $data_query['limit'];
		$this->pagination->initialize($config);
		// END PAGINATION //
		$this->render_admin('admin/' . $this->uri->segment(2) . '/index', ['table' => $this->api_service_model->get_rows($data_query), 'total_data' => $config['total_rows'], 'field' => $field, 'operator' => $operator]);
	}
	public function page()
	{
		$this->render_admin('admin/' . $this->uri->segment(2) . '/page');
	}
	public function form($i = '')
	{
		$target = $this->api_service_model->get_row(['api_id' => $i]);
		if ($target == false) { // add
			if ($this->input->post()) {
				...
				$this->form_validation->set_rules('service_id_key', 'Response Service ID', 'required');
				$this->form_validation->set_rules('category_key', 'Response Service Cetegory', 'required');
				$this->form_validation->set_rules('service_name_key', 'Response Service Name', 'required');
				$this->form_validation->set_rules('min_key', 'Response Service Min', 'required');
				$this->form_validation->set_rules('max_key', 'Response Service Max', 'required');
				$this->form_validation->set_rules('price_key', 'Response Service Price', 'required');
				$this->form_validation->set_rules('refill_key', 'Response Service Refill', 'required');
				$data_input = [
					...
					'service_id_key' => $this->input->post('service_id_key'),
					'category_key' => $this->input->post('category_key'),
					'service_name_key' => $this->input->post('service_name_key'),
					'description_key' => null,
					'min_key' => $this->input->post('min_key'),
					'max_key' => $this->input->post('max_key'),
					'price_key' => $this->input->post('price_key'),
					'refill_key' => $this->input->post('refill_key'),
				...
				if ($this->form_validation->run() == false) {
					...
					} elseif ($this->api_service_model->get_row(['api_id' => $data_input['api_id']])) {
						$this->session->set_flashdata('result', array('alert' => 'danger', 'title' => 'Gagal!', 'msg' => 'Nama Provider sudah ada didatabase.'));
					} else {
						$data_input['description_key'] = ($this->input->post('description_key') <> '') ? $this->input->post('description_key') : '-';
						$data_input['data_service_key'] = ($this->input->post('data_service_key') <> '') ? $this->input->post('data_service_key') : '-';
						$insert_data = $this->api_service_model->insert($data_input);
						if ($insert_data) {
							...
							$this->session->set_flashdata('result', array('alert' => 'success', 'title' => 'Yeay!', 'msg' => 'Parameter Service Provider <b>#' . $data_input['api_id'] . '</b> berhasil ditambahkan.'));
			...
			if ($this->input->post()) {
				...
				$this->form_validation->set_rules('method', 'HTTP Method', 'required');
				$this->form_validation->set_rules('service_id_key', 'Response Service ID', 'required');
				$this->form_validation->set_rules('category_key', 'Response Service Category', 'required');
				$this->form_validation->set_rules('service_name_key', 'Response Service Name', 'required');
				$this->form_validation->set_rules('min_key', 'Response Service Min', 'required');
				$this->form_validation->set_rules('max_key', 'Response Service Max', 'required');
				$this->form_validation->set_rules('price_key', 'Response Service Price', 'required');
				...
				if ($this->form_validation->run() == false) {
					...
					if ($this->api_model->get_row(['id' => $data_input['api_id']]) == false) {
						...
						$data_input['description_key'] = ($this->input->post('description_key') <> '') ? $this->input->post('description_key') : '-';
						$data_input['data_service_key'] = ($this->input->post('data_service_key') <> '') ? $this->input->post('data_service_key') : '-';
						$this->api_service_model->update($data_input, ['api_id' => $target->api_id]);
						$this->api_request_param_model->delete(['api_id' => $i, 'api_type' => 'service']);
						...
						$this->session->set_flashdata('result', array('alert' => 'success', 'title' => 'Yeay!', 'msg' => 'Paramter Service Provider <b>#' . $target->api_id . '</b> berhasil diperbaharui.'));
						redirect(base_url('admin/api'));
						exit();
					}
				}
			...
			$this->load->view('admin/' . $this->uri->segment(2) . '/edit', ['target' => $target, 'api' => $this->api_model->get_rows(), 'api_request_param' => $this->api_request_param_model->get_rows(['where' => [['api_id' => $i, 'api_type' => 'service']]])]);
			// $this->render_admin('admin/' . $this->uri->segment(2) . '/edit', ['target' => $target, 'api' => $this->api_model->get_rows(), 'api_request_param' => $this->api_request_param_model->get_rows(['where' => [['api_id' => $i, 'api_type' => 'service']]])]);
		}
	}
	public function delete($i = '')
	{
		...
		$delete_target = $this->api_service_model->delete(['api_id' => $i]);
		if ($delete_target) {
			$this->api_request_param_model->delete(['api_id' => $i, 'api_type' => 'service']);
