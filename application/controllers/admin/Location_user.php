...
class Location_user extends MY_Controller
{
	...
	public function index()
	{
		...
		$field = [
			...
			'location.langtitude' => 'LANGTITUDE',
			'location.longtitude' => 'LONGTITUDE',
			'location.tangga' => 'TANGGAL & WAKTU',
		...
		// SETTINGS //
		$data_query = [
			'select' => 'location.*, user.username as username',
			'join' => [
				[
					'table' => 'user',
					'on' => 'user.id = location.id_user',
					'param' => 'inner'
				]
			],
		...
		$config['total_rows'] = $this->location_model->get_count($data_query);
		$config['per_page'] = $data_query['limit'];
		$this->pagination->initialize($config);
		// END PAGINATION //
		$this->render_admin('admin/' . $this->uri->segment(2) . '/index', ['table' => $this->location_model->get_rows($data_query), 'total_data' => $config['total_rows'], 'field' => $field, 'operator' => $operator, 'service_category' => $this->service_category_model->get_rows(), 'api' => $this->api_model->get_rows()]);
	}
}
