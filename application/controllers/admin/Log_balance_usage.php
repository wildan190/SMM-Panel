...
class Log_balance_usage extends MY_Controller
{
	...
	public function index()
	{
		// FORM INPUT //
		$field = [
			'log_balance_usage.id' => 'ID',
			'user.username' => 'PENGGUNA',
			'log_balance_usage.type' => 'TIPE',
			'log_balance_usage.category' => 'KATEGORI',
			'log_balance_usage.amount' => 'JUMLAH',
			'log_balance_usage.description' => 'DESKRIPSI',
			'log_balance_usage.created_at' => 'TANGGAL/WAKTU',
		];
		$operator = [
			'equal' => 'WHERE =',
			'not_equal' => 'WHERE <>',
		...
		// SETTINGS //
		$data_query = [
			'select' => 'log_balance_usage.*, user.username',
			'join' => [
				[
					'table' => 'user',
					'on' => 'user.id = log_balance_usage.user_id',
					'param' => 'inner'
				]
			],
			'order_by' => 'log_balance_usage.id DESC',
			'limit' => '30',
		...
		// END PAGINATION //
		$widget = [
			'plus' => $this->log_balance_usage_model->get_rows(['select' => 'SUM(amount) AS rupiah, COUNT(amount) AS total', 'where' => [['type' => 'plus']]]),
			'minus' => $this->log_balance_usage_model->get_rows(['select' => 'SUM(amount) AS rupiah, COUNT(amount) AS total', 'where' => [['type' => 'minus']]]),
		];
		$this->render_admin('admin/' . $this->uri->segment(2) . '/index', ['table' => $this->log_balance_usage_model->get_rows($data_query), 'total_data' => $config['total_rows'], 'field' => $field, 'operator' => $operator, 'widget' => $widget]);
	}
}
