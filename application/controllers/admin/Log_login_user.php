...
class Log_login_user extends MY_Controller
{
	...
	public function index()
	{
		// FORM INPUT //
		$field = [
			'log_login_user.id' => 'ID',
			'user.username' => 'PENGGUNA',
			'log_login_user.ip_address' => 'ALAMAT IP',
			'log_login_user.created_at' => 'TANGGAL/WAKTU',
		];
		$operator = [
			'equal' => 'WHERE =',
			'not_equal' => 'WHERE <>',
		...
		// SETTINGS //
		$data_query = [
			'select' => 'log_login_user.*, user.username',
			'join' => [
				[
					'table' => 'user',
					'on' => 'user.id = log_login_user.user_id',
					'param' => 'inner'
				]
			],
			'order_by' => 'log_login_user.id DESC',
			'limit' => '30',
		...
		// END PAGINATION //
		$this->render_admin('admin/' . $this->uri->segment(2) . '/index', ['table' => $this->log_login_user_model->get_rows($data_query), 'total_data' => $config['total_rows'], 'field' => $field, 'operator' => $operator]);
	}
}
