...
class Replace_keyword extends MY_Controller
{
	...
	public function index()
	{
		...
		$config['total_rows'] = $this->replace_keyword_model->get_count($data_query);
		$config['per_page'] = $data_query['limit'];
		$this->pagination->initialize($config);
		// END PAGINATION //
		$this->render_admin('admin/' . $this->uri->segment(2) . '/index', ['table' => $this->replace_keyword_model->get_rows($data_query), 'total_data' => $config['total_rows'], 'field' => $field, 'operator' => $operator]);
	}
	public function form($i = '')
	{
		$target = $this->replace_keyword_model->get_by_id($i);
		//$this->lib->print_data($this->input->post());
		if ($target == false) { // add

			if ($this->input->post()) {
				$this->form_validation->set_rules('name', 'Kata Kunci', 'required|min_length[1]|max_length[150]');
				$this->form_validation->set_rules('target', 'Kata Ganti', 'required');
				if ($this->form_validation->run() == true) {
					$data_input = [
						'name' => $this->db->escape_str($this->input->post('name')),
						'target' => $this->input->post('target'),
					];
					if ($this->replace_keyword_model->get_row(['name' => $data_input['name']])) {
						$this->session->set_flashdata('result', array('alert' => 'danger', 'title' => 'Gagal!', 'msg' => 'Nama sudah ada didatabase.'));
					} else {
						$insert_data = $this->replace_keyword_model->insert($data_input);
						if ($insert_data) {
			...
			if ($this->input->post()) {
				...
				if ($this->form_validation->run() == true) {
					...
					if ($data_input['name'] <> $target->name and $this->replace_keyword_model->get_row(['name' => $data_input['name']])) {
						$this->session->set_flashdata('result', array('alert' => 'danger', 'title' => 'Gagal!', 'msg' => 'Nama sudah ada didatabase.'));
					} else {
						$update_target = $this->replace_keyword_model->update($data_input, ['id' => $i]);
						if ($update_target) {
	...
	public function delete($i = '')
	{
		...
		$delete_target = $this->replace_keyword_model->delete(['id' => $i]);
		if ($delete_target) {
