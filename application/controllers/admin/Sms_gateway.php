...
class Sms_gateway extends MY_Controller
{
	...
	public function index()
	{
		...
		$config['total_rows'] = $this->sms_gateway_model->get_count($data_query);
		$config['per_page'] = $data_query['limit'];
		$this->pagination->initialize($config);
		// END PAGINATION //
		$this->render_admin('admin/' . $this->uri->segment(2) . '/index', ['table' => $this->sms_gateway_model->get_rows($data_query), 'total_data' => $config['total_rows'], 'field' => $field, 'operator' => $operator]);
	}
	public function form($i = '')
	{
		$target = $this->page_model->get_by_id($i);
		if ($target == false) { // add
			if ($this->input->post()) {
				$this->form_validation->set_rules('target', 'Target', 'required|alpha_numeric_spaces|numeric');
				...
				if ($this->form_validation->run() == true) {
					...
					$header = [
						...
						'Authorization: eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJhZG1pbiIsImlhdCI6MTYwMDc0MTE1NiwiZXhwIjo0MTAyNDQ0ODAwLCJ1aWQiOjgzOTM1LCJyb2xlcyI6WyJST0xFX1VTRVIiXX0.Px40n02HGrHgFCwgWojAceT85fm742VuGOLQyvlFf0o'
					];
					$body = '[
      					{
      						"phone_number": ' . $data_input['target'] . ', 
      						"message": ' . $data_input['msg'] . ', 
      						"device_id": 120620
      					}
      				]';
					$curl = $this->lib->curl("https://smsgateway.me/api/v4/message/send", $body, $header);
					log_message('error', $curl);

					$response = json_decode($curl, true);

					if ($response['status'] == 'fail') {
						$this->session->set_flashdata('result', array('alert' => 'danger', 'title' => 'Gagal!', 'msg' => $response['message']));
					} else {
						$insert_data = $this->sms_gateway_model->insert($data_input);
						if ($insert_data) {
			...
			if ($this->input->post()) {
				$this->form_validation->set_rules('title', 'Judul', 'required|alpha_numeric_spaces|min_length[1]|max_length[100]');
				$this->form_validation->set_rules('content', 'Konten', 'required');
				if ($this->form_validation->run() == true) {
					$data_input = [
						'title' => $this->db->escape_str($this->input->post('title')),
						'slug' => url_title($this->db->escape_str($this->input->post('title')), 'dash', true),
						'content' => $this->input->post('content'),
						'updated_at' => date('Y-m-d H:i:s'),
					];
					if ($data_input['title'] <> $target->title and $this->page_model->get_row(['title' => $data_input['title']])) {
						$this->session->set_flashdata('result', array('alert' => 'danger', 'title' => 'Gagal!', 'msg' => 'Judul sudah ada didatabase.'));
					} else {
						$update_target = $this->page_model->update($data_input, ['id' => $i]);
						if ($update_target) {
	...
	public function delete($i = '')
	{
		...
		$delete_target = $this->page_model->delete(['id' => $i]);
		if ($delete_target) {
