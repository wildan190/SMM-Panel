...
class Type_category extends MY_Controller
{
	...
	public function index()
	{
		...
		$config['total_rows'] = $this->type_category_model->get_count($data_query);
		$config['per_page'] = $data_query['limit'];
		$this->pagination->initialize($config);
		// END PAGINATION //
		$this->render_admin('admin/' . $this->uri->segment(2) . '/index', ['table' => $this->type_category_model->get_rows($data_query), 'total_data' => $config['total_rows'], 'field' => $field, 'operator' => $operator]);
	}

	public function form($i = '')
	{
		$target = $this->type_category_model->get_by_id($i);
		if ($target == false) { // add
			if ($this->input->post()) {
				$this->form_validation->set_rules('type', 'Tipe', 'required|min_length[1]|max_length[100]');
				if ($this->form_validation->run() == true) {
					$data_input = [
						'type' => $this->db->escape_str($this->input->post('type'))
					];
					if ($this->type_category_model->get_row(['type' => $data_input['type']])) {
						$this->session->set_flashdata('result', array('alert' => 'danger', 'title' => 'Gagal!', 'msg' => 'Nama sudah ada didatabase.'));
					} else {
						$insert_data = $this->type_category_model->insert($data_input);
						if ($insert_data) {
			...
			if ($this->input->post()) {
				...
				if ($this->form_validation->run() == true) {
					...
					if ($data_input['name'] <> $target->name and $this->type_category_model->get_row(['name' => $data_input['name'], 'type' => $data_input['type']])) {
						$this->session->set_flashdata('result', array('alert' => 'danger', 'title' => 'Gagal!', 'msg' => 'Nama sudah ada didatabase.'));
					} else {
						$update_target = $this->type_category_model->update($data_input, ['id' => $i]);
						if ($update_target) {
	...
	public function delete($i = '')
	{
		...
		$delete_target = $this->type_category_model->delete(['id' => $i]);
		if ($delete_target) {
	...
	{
		if ($this->type_category_model->is_empty()) {
			$this->session->set_flashdata('result', array('alert' => 'success', 'title' => 'Perubahan status berhasil!', 'msg' => 'Semua kategori layanan berhasil di hapus'));
		} else {
			$this->session->set_flashdata('result', array('alert' => 'danger', 'title' => 'Gagal!', 'msg' => 'Kesalahan tidak terduga.'));
		}
		redirect(base_url('admin/' . $this->uri->segment(2)));
