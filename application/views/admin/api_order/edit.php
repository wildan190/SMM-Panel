...
<form method="post" action="<?= base_url('admin/api_order/form/' . $target->api_id) ?>">
    ...
    <div class="row">
        <div class="col-md-12">
                ...
                <div id="parameter">
                    ...
                    ?>
                            <div class="row" id="form_<?= $value['id'] ?>">
                                ...
                                <div class="form-group col-4"><select class="select2-data form-control" id="param_type_<?= $value['id'] ?>" name="param_type[]" onchange="param_type(<?= $value['id'] ?>, this.value)">
                                ...
                                <div class="form-group col-4" id="form_pv_<?= $value['id'] ?>">
                                    <?php if ($value['param_type'] == 'primary') { ?>
                                        <select class="select2-data form-control" id="param_value_<?= $value['id'] ?>" name="param_value[]">
                                            ...
                                            <option value="service_id" <?= ($value['param_value'] == 'service_id') ? 'selected' : ''; ?>>Service</option>
                                            <option value="target" <?= ($value['param_value'] == 'target') ? 'selected' : ''; ?>>Target</option>
                                            <option value="quantity" <?= ($value['param_value'] == 'quantity') ? 'selected' : ''; ?>>Quantity</option>
                                            <option value="custom_comment" <?= ($value['param_value'] == 'custom_comment') ? 'selected' : ''; ?>>Custom Comment</option>
                                            <option value="custom_link" <?= ($value['param_value'] == 'custom_link') ? 'selected' : ''; ?>>Custom Link</option>
                                        </select>
                                    <?php } else { ?>
            ...
            <div class="form-group">
                <label for="order_id_key" class="form-label">Respond ID Pesanan <span class="text-danger">*</span></label>
                <input type="text" class="form-control" value="<?= $target->order_id_key ?>" id="order_id_key" name="order_id_key" placeholder="Contoh: id, order_id">
            </div>
...
<script type="text/javascript">
    ...
    function param_type(key, value) {
        if (value == 'primary') {
            $('#form_pv_' + key).html('<select class="select2-data form-control" id="param_value_' + key + '" name="param_value[]"><option value="api_id">API ID</option><option value="api_key">API Key</option><option value="secret_key">Secret Key</option><option value="service_id">Service</option><option value="target">Target</option><option value="quantity">Quantity</option><option value="custom_comment">Custom Comment</option><option value="custom_link">Custom Link</option></select>');
            $("select#param_value_" + key).each((_i, e) => {
                var $e = $(e);
                $e.select2({
                    dropdownParent: $e.parent()
    ...
    function add_response_oid() {
        $('#response_oid').append('<input type="text" class="form-control input_oid" name="response_oid[]">');
    }

    function remove_response_oid() {
        var response_api = $('.input_oid');
        if (response_api.length > 0) {
            response_api.each((key, value) => {
                if (response_api.length - 1 == key) {
                    $(value).remove();
    ...
    function change_response() {
        if ($('#use_response_oid').is(':checked')) {
