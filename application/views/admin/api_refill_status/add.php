<form class="form-horizontal" method="post" action="<?= base_url('admin/api_refill_status/form') ?>">
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="form-group">
                        ...
                        <textarea class="form-control" id="success_response" style="height: 150px;" name="success_response"><?= set_value('order_success_response') ?></textarea>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Response Status <span class="text-danger">*</span></label>
                        <input type="text" class="form-control" name="refill_id_key">
                    </div>
                </div>
            </div>
        </div>
...
<script type="text/javascript">
    ...
    function param_type(key, value) {
        if (value == 'primary') {
            $('#form_pv_' + key).html('<select class="select2-data form-control" id="param_value_' + key + '" name="param_value[]"><option value="api_id">API ID</option><option value="api_key">API Key</option><option value="secret_key">Secret Key</option><option value="refill_id">Refill ID</option></select>');
            $("select#param_value_" + key).each((_i, e) => {
                var $e = $(e);
                $e.select2({
                    dropdownParent: $e.parent()
