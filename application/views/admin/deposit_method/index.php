					...
					<div class="col-md-12">
						...
						<div class="card">
							<div class="card-header">
								<h4 class="mb-0"><i class="far fa-credit-card me-2"></i>Metode Deposit</h4>
							</div>
							<div class="card-body">
									...
									<table class="table table-nowrap table-bordered table-striped align-middle table-hover m-0" id="table-data">
										<thead class="text-muted table-light">
											<tr class="">
												...
												<th>TYPE</th>
												<th>KODE</th>
												<th>GAMBAR DEPOSIT</th>
												<th>NO REKENING/HP</th>
												<th>ATAS NAMA</th>
												<th>RATE</th>
												<th width="15%">MINIMAL DEPOSIT</th>
												<th>STATUS</th>
												<th>AKSI</th>
											</tr>
										</thead>
										<tbody>
											...
											?>
												<tr>
													...
													<td><?= $value['name'] ?></td>
													<td><?= $value['type'] ?></td>
													<td><?= $value['gateway_code'] ?></td>
													<td class="table-primary align-middle text-center"><img src="<?= base_url() ?>uploads/<?= $value['thumbnail_payment'] ?>" alt="Payment" width="80"></td>
													<td><?= $value['number_account'] ?></td>
													<td><?= $value['name_account'] ?></td>
													<td><?= $value['rate'] ?></td>
													<td>Rp <?= number_format($value['min_deposit'], 0, ',', '.') ?></td>
													<td align="center">
														<div class="btn-group">
													...
													<td align="center">
														<a href="javascript:;" onclick="modal_open('detail', 'lg', '<?= base_url('admin/' . $this->uri->segment(2) . '/detail/' . $value['id']) ?>')" class="btn btn-sm btn-primary round btn-sm" data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" title="Detail"><i class="fas fa-search fs-6 me-2"></i>Detail</a>
														<a href="<?= base_url('admin/' . $this->uri->segment(2) . '/form/' . $value['id']) ?>" class="btn btn-sm btn-warning round btn-sm me-1" data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" title="Ubah"><i class="fas fa-edit fs-6 me-2"></i>Ubah</a>
														<a href="javascript:;" onclick="confirm_hapus('<?= $value['id'] ?>')" class="btn btn-sm btn-danger round btn-sm" data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" title="Hapus"><i class="fas fa-trash fs-6 me-2"></i>Hapus</a>
													</td>
												</tr>
											<?php
											}
				...
				<script type="text/javascript">
					function confirm_hapus(id) {
						Swal.fire({
							title: 'Yakin ingin menghapus Data Metode Deposit #' + id + '?',
							type: 'question',
							...
							if (result.value) window.location = "<?= base_url() ?>admin/deposit_method/delete/" + id
						})
					}
				</script>