            ...
            <div class="card-body pb-4">
                ...
                <div class="table-responsive">
                    <?php foreach ($widget as $status => $data) : ?>
                        <table class="table <?= ($status == 'success') ? 'table-success' : (($status == 'pending') ? 'table-warning' : (($status == 'processing') ? 'table-info' : (($status == 'error' || $status == 'partial') ? 'table-danger' : ''))) ?> table-hover mb-0">
                            <tr>
                                <th width="30%"><?= ucfirst($status) ?></th>
                                <td>Rp <?= number_format($data[0]['rupiah'], 0, ',', '.') ?> dari <?= number_format($data[0]['total'], 0, ',', '.') ?> pesanan.</td>
                            </tr>
                        </table>
                    <?php endforeach; ?>
                </div>
...
<script>
    ...
    const chartData = Object.keys(jsonData).map(date => ({
        ...
        processing: parseInt(jsonData[date].processing),
        error: parseInt(jsonData[date].error),
        partial: parseInt(jsonData[date].partial),
    }));
