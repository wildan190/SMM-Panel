...
<div class="row">
    ...
    <div class="row">
    <div class="col-md-12">
        <a class="btn mb-3 btn-primary bg-gradient" data-bs-toggle="collapse" href="#filterCategory">
                ...
                <form method="POST">
                        ...
                        <div class="tab-pane active" id="general" role="tabpanel">
                            <div class="form-group">
                                ...
                                <select class="select2 form-control" id="category">
                                    ...
                                    ?>
                                        <option value="<?= $value['id'] ?>" <?= (isset($prev_input['category_id']) && $prev_input['category_id'] == $value['id']) ? 'selected' : '' ?> data-icon="<?= $this->lib->dataicon($value['name']) ?>"><?= $value['name'] ?></option>
                                    <?php
                                    }
                                    ?>
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <select class="select2 form-control" id="category_fav">
                                    ...
                                    <?php
                                        ...
                                        ?> <option value="<?= $value['category_id'] ?>" data-icon="<?= $this->lib->dataicon($value['category_name']) ?>"><?= $value['category_name'] ?></option>
                                    <?php
                                        }
                                    ?>
                                </select>
                        ...
                        <div class="tab-pane" id="search" role="tabpanel">
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    ...
                                    <a href="javascript:;" class="btn btn-primary bg-gradient" id="btnSearch" style="padding-top: 0.8rem;">Cari</a>
                                </div>
                            </div>
                    ...
                    <div class="row">
                        <div class="form-group col-md-6">
                            ...
                            <input type="text" class="form-control" name="target" id="target" value="<?= (isset($prev_input['target'])) ? $prev_input['target'] : '' ?>">
                        </div>
                        <div class="form-group col-md-6">
                            <label class="form-label">Jumlah Pesanan <span class="text-danger">*</span></label>
                            <input type="number" class="form-control" name="quantity" id="quantity" value="<?= (isset($prev_input['quantity'])) ? $prev_input['quantity'] : '' ?>">
                        </div>
                    </div>
                    ...
                    <div class="form-group d-none" id="formCommentLikes">
                        ...
                        <input type="text" class="form-control" id="inputCommentLikes" value="<?= (isset($prev_input['username'])) ? $prev_input['username'] : '' ?>" placeholder="Nama pengguna pembuat komentar..."><span class="text-danger">*Nama pengguna pembuat komentar yang ingin disukai.</span>
                    </div>
                    ...
                    <div class="mb-0">
                        <button type="submit" class="btn btn-primary bg-gradient float-end"><i class="fas fa-cart-plus fs-6 me-2"></i>Pesan</button>
                        <button type="reset" class="btn btn-danger bg-gradient float-end me-2"><i class="fas fa-sync fs-6 me-2"></i>Ulangi</button>
                    </div>
                </form>
            </div>
        </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data.status == false) {
                        // Logika jika layanan tidak ditemukan
                        ...
                        if (data.type == 'primary') {
                            ...
                            $('#total-price').val(data.msg.price);
                        } else if (data.msg.type == 'SEO') {
                            $('#formSEO').removeClass('d-none');
                            $('#inputSEO').attr('name', 'keywords');
                        } else if (data.msg.type == 'custom_comments') {
                            if (data.msg.max == '1' || data.msg.name.toLowerCase().includes('package')) {
                                $('#formCustomCommentsPackage').removeClass('d-none');
                                $('#inputCustomCommentsPackage').attr('name', 'custom_comments');
                                $('#quantity').attr('disabled', true);
                                $('#quantity').val(1);
                                $('#label-price').html('Harga / 1');
                                $('#total-price').val(data.msg.price);
                                ...
                                $('#inputCustomComments').attr('name', 'custom_comments');
                                $('#quantity').prop('readonly', true);
                            }
                        ...
                        } else if (data.msg.type == 'Mentions Custom List') {
                            ...
                            $('#inputMentionsMediaLikers').attr('name', 'media');
                        } else if (data.msg.type == 'Custom Comments Package') {
                            $('#formCustomCommentsPackage').removeClass('d-none');
                            ...
                            $('#label-price').html('Harga / 1');
                            $('#total-price').val(data.msg.price);
                        } else if (data.msg.type == 'Comment Likes' || data.msg.name.toLowerCase().includes('comment likes') || data.msg.name.toLowerCase().includes('likes comment')) {
                        ...
                        } else if (data.msg.type == 'Comment Replies') {
                ...
                error: function() {
                    HoldOn.close();
                    $('#ajax-result').html('<font color="red">Terjadi kesalahan! Silakan refresh halaman.</font>');
                },
                beforeSend: function() {
                    HoldOn.open({
                        theme: "sk-rect",
        ...
        // Fungsi untuk menangani perhitungan total harga saat mengubah jumlah
        var totalPriceDelayTimer;
        $('#quantity').keyup(function() {
            clearTimeout(totalPriceDelayTimer);
            totalPriceDelayTimer = setTimeout(function() {
                var service = $('#service').val();
                var inputQuantity = $('#quantity').val();
                var quantity = parseInt(inputQuantity.replace(/^0+/, ''), 10);
                if (isNaN(quantity)) {
                    quantity = 0;
                }
        ...
        $('#inputSEO').keyup(function() {
            $('#inputSEOAlert').html('*Keywords list separated by Enter.');
        });
        $('#inputCustomComments').keyup(function() {
            var area = document.getElementById("inputCustomComments")
            var text = area.value.replace(/\s+$/g, "")
            var split = text.split("\n")
            $('#inputCustomCommentsAlert').html('*Daftar komentar dipisahkan dengan Enter.');
            $('#quantity').val(split.length);
            var service = $('#service').val();
            var quantity = $('#quantity').val();
            total_price(service, quantity);
        });
        $('#inputMentions').keyup(function() {
            $('#inputMentionsAlert').html('*Usernames list separated by Enter.');
        });
        $('#inputMentionsWithHashtags1').keyup(function() {
            $('#inputMentionsWithHashtags1Alert').html('*Usernames list separated by Enter.');
        });
        $('#inputMentionsWithHashtags2').keyup(function() {
            $('#inputMentionsWithHashtags2Alert').html('*Hashtags list separated by Enter.');
        });
        $('#inputMentionsCustomList').keyup(function() {
            var area = document.getElementById("inputMentionsCustomList")
            var text = area.value.replace(/\s+$/g, "")
            var split = text.split("\n")
            $('#inputMentionsCustomListAlert').html('*Usernames list separated by Enter.');
            $('#quantity').val(split.length);
            var service = $('#service').val();
            var quantity = $('#quantity').val();
            total_price(service, quantity);
        ...
        $('#inputCustomCommentsPackage').keyup(function() {
            $('#inputCustomCommentsPackageAlert').html('*Comments list separated by Enter.');
        });
        $('#inputCommentReplies2').keyup(function() {
            var area = document.getElementById("inputCommentReplies2")
            var text = area.value.replace(/\s+$/g, "")
            var split = text.split("\n")
            $('#inputCommentReplies2Alert').html('*Comments list separated by Enter.');
            $('#quantity').val(split.length);
            var service = $('#service').val();
            var quantity = $('#quantity').val();
            total_price(service, quantity);
        });
        $('#inputInvitesFromGroups').keyup(function() {
            // $('#inputInvitesFromGroupsAlert').html('*The message you want to send.');
        });
